...
func JSON[T any]() *RuleSet {
	return &RuleSet{
		RuleName: "safe.JSONParsable",
		MessageFunc: func(rs *RuleSet) string {
			return messages.InvalidFormatMsg(rs.Language)
		},
		ValidateFunc: func(rs *RuleSet) bool {
			bytes, ok := rs.FieldValue.([]byte)
			if !ok {
				return false
			}
...
// The field must be a []byte with a valid format for JSONInto.
// It should be parsable to T
//
// In order to avoid unecessary parsing, you can pass a dst[*T] arg.
func JSONInto[T any](dst *T) *RuleSet {
	return &RuleSet{
		RuleName: "safe.JSON",
		MessageFunc: func(rs *RuleSet) string {
			return messages.InvalidFormatMsg(rs.Language)
